...
class WebResourceCollector {
  ...
  _mapWebResourceById (relatedResources, webResourceLinksByWebResourceId) {
    ...
    let webResourceBySfdcId = _.keyBy(_.cloneDeep(_.compact(webResources)), 'Id')
  ...
  _mapWebResourcesByProductId (relatedResources, webResourcesById) {
    ...
    _.forEach(relatedResources, (relatedResource) => {
      ...
      this._addWebResourceByType(
        webResources,
        webResourcesById[relatedResource.Primary_Web_Resource__c])
      this._addWebResourceByType(
        webResources,
        webResourcesById[relatedResource.Related_Web_Resource__c])

      webResourcesByProductId[productId] = webResources
    })
    return webResourcesByProductId
  }

  _addWebResourceByType (webResources, webResource) {
    if (!_.isEmpty(webResource)) {
      let type = webResource.Type__c
      let webResourcesByType = webResources[type] || []
      webResourcesByType.push(webResource)
      webResources[type] = _.uniqBy(webResourcesByType, 'Id')
    }
  }
}
