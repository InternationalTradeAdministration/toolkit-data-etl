...
getObjects = () => {
	_.forEach(config, (config_entry) => {
		const query_result = {};
		getFilterGroups(query_result, config_entry);
	});
}

getFilterGroups = (query_result, config_entry) => {
	conn.apex.post('/ToolkitSearch/', {programId: config_entry.program_id, languageIso: config_entry.language }, (err, res) => {
		if (err) { return console.error(err); }
		query_result.filter_groups = res.filterGroups;
		getProviders(query_result, config_entry);
	});
}
...
getProviders = (query_result, config_entry) => {
	conn.query(`SELECT ID, Name, Company_Description__c FROM Participation__c WHERE Program__c IN ('${config_entry.program_id}')`, (err, res) => {
		...
		getSolutionsWithProviders(query_result, config_entry);
	});
}
